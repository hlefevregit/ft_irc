...
/*   Updated: 2025/01/29 16:59:34 by hulefevr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int    Server::parseExec(Client *client, std::string const &message, std::map<const int, Client>::iterator it)
{
...
int    Server::parseMessage(Client *client, std::string const &message)
{
    ...
    for (size_t i = 0; i != cmds.size(); i++)
    {
        ...
        {
            if (parseExec(client, message, it) == 1)
                std::cout << RED << "[ERROR]" << RESET << " Command not found" << std::endl;
